...
const ChatModule: React.FC = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      // Production API endpoint as requested
      const response = await fetch('https://tapal.store/api/chat', {
        method: 'POST',
        headers: {
          ...
          {messages.map((msg) => (
            ...
            >
              <div className={`flex max-w-[80%] space-x-3 ${msg.role === 'user' ? 'flex-row-reverse space-x-reverse' : 'flex-row'}`}>
                ...
                <div className={`p-4 rounded-2xl shadow-sm text-sm leading-relaxed ${msg.role === 'user' ? 'bg-amber-500 text-amber-950 font-bold rounded-tr-none' : 'bg-white text-stone-800 border border-stone-200 rounded-tl-none'}`}>
                  {msg.content}
                </div>
              </div>
            </motion.div>
      ...
      <div className="p-6 bg-white border-t border-stone-100">
        <form onSubmit={handleSendMessage} className="relative flex items-center">
          <input
            ...
            className="w-full bg-stone-100 border-none rounded-2xl py-4 pl-6 pr-16 text-stone-800 placeholder-stone-400 focus:ring-2 focus:ring-amber-500 transition-all font-medium"
            disabled={isLoading}
          />
          <button
            type="submit"
