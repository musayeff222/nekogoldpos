...
const ReturnsModule: React.FC<ReturnsProps> = ({ sales, setSales, products, setProducts, addLog }) => {
  ...
  return (
    <div className="flex flex-col space-y-6 pb-24 md:pb-10 animate-in fade-in duration-500">
      {/* TABS */}
      <div className="flex space-x-2 bg-white p-2 rounded-2xl border border-stone-100 shadow-sm self-start">
        <button 
          onClick={() => setActiveTab('process')}
          className={`px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'process' ? 'bg-stone-900 text-amber-500 shadow-lg' : 'text-stone-400 hover:bg-stone-50'}`}
        >
          İadə Et
        </button>
        <button 
          onClick={() => setActiveTab('list')}
          className={`px-6 py-2.5 rounded-xl font-black text-[10px] uppercase tracking-widest transition-all ${activeTab === 'list' ? 'bg-stone-900 text-amber-500 shadow-lg' : 'text-stone-400 hover:bg-stone-50'}`}
        >
          İadələr Siyahısı
        </button>
      </div>
      ...
      {activeTab === 'process' ? (
        ...
        <div className="bg-white rounded-[2.5rem] md:rounded-[3rem] border border-stone-100 shadow-2xl overflow-hidden animate-in slide-in-from-bottom-4 duration-500">
          <div className="p-8 md:p-12 border-b border-stone-50 bg-stone-50/30 flex items-center justify-between">
            <div>
              <h3 className="text-xl font-black text-stone-900 uppercase tracking-tighter">İadələr Siyahısı</h3>
              <p className="text-xs text-stone-400 font-bold mt-1 uppercase tracking-widest">Geri qaytarılmış və stoka hələ əlavə edilməmiş məhsullar</p>
            </div>
            <div className="bg-amber-500 text-stone-900 px-6 py-3 rounded-2xl font-black text-lg shadow-lg">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-stone-50/50">
                  ...
                  <th className="px-8 py-6 text-[10px] font-black text-stone-400 uppercase tracking-widest">Çəki</th>
                  <th className="px-8 py-6 text-[10px] font-black text-stone-400 uppercase tracking-widest">Səbəb / Qeyd</th>
                  <th className="px-8 py-6 text-[10px] font-black text-stone-400 uppercase tracking-widest text-right">Əməliyyat</th>
                </tr>
              </thead>
                  ...
                  <tr key={p.id} className="hover:bg-amber-50/30 transition-colors group">
                    <td className="px-8 py-6">
                      <div className="flex items-center space-x-4">
                        <div className="w-12 h-12 bg-stone-50 rounded-xl flex items-center justify-center text-amber-500 border border-stone-100 overflow-hidden">
                          {p.imageUrl ? (
                        ...
                        <span className="font-black text-stone-800 uppercase text-sm">{p.name}</span>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <span className="text-[10px] font-black text-amber-600 bg-amber-50 px-2 py-1 rounded uppercase tracking-widest">{p.code}</span>
                    </td>
                    <td className="px-8 py-6 font-bold text-stone-500 text-sm">{p.weight} gr</td>
                    <td className="px-8 py-6">
                      <p className="text-[10px] font-bold text-stone-400 uppercase leading-relaxed max-w-xs">{p.returnReason || 'Qeyd yoxdur'}</p>
                    </td>
                    <td className="px-8 py-6 text-right">
                      <button 
                        onClick={() => handleRestoreToStock(p)}
                        className="bg-stone-900 text-amber-500 px-4 py-2 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-black transition-all shadow-md flex items-center space-x-2 ml-auto"
                      >
                        <RefreshCw size={14} />
